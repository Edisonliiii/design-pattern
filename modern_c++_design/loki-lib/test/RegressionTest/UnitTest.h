...
private:
  ...
  template<class>
  struct In { enum { value = false }; };
  // if T1 == T2, will go here
  template<>
  struct In<T1> { enum { value = true }; };
public:
...
protected:
  ...
  {
    if (name.length() != 0) std::cout << std::string(result.pos, ' ') << name << std::endl;
  }

  void testAssert(
    const std::string& s, bool assertion, TestResult& result, bool supported = true,
    const std::string& failStr = emptyStr())
  {
    ...
    if (supported)
    {
      ...
      {
        std::cout << str << (failStr == emptyStr() ? std::string("Failed") : "Failed - " + failStr) << std::endl;
        ++result.failed;
      }
    }
    else
...
public:
  ...
  {
    std::cout << title << std::string(Test::offset - title.length(), ' ') << "Result" << std::endl;
