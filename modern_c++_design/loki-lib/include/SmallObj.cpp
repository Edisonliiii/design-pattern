...
class Chunk {
 private:
  ...
  /** Determines if the Chunk has been corrupted.(check-wise check)
    ...
    blocks for corruption.  If false, then caller wants to skip some tests tests
    just to run faster.  (Debug version does more checks, but release version
    runs faster.)
