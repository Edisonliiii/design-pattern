 ...
 * - Reference: if T is a reference to some type, ParameterType is the same as T
 *              because references to references are not allowed
 *
...
class TypeTraits {
 private:
  ...
  // ------------- area for FunctionPointerTraits
  template <class U>
  ...
  // there are already a bounch of IsMemberFunctionPointerRaw ready to use
  template <typename U>
 ...
 public:
  ...
  // first strip off const, then volatile
  typedef
