...
namespace Loki {
  ...
  namespace TL {  // typelist utility algorithms
    ...
    typedef TypeList<
      ...
      SingedIntegrals;
    // Here, using recursive way to define a typelist, will benefit code
    // reuseability
...
// Therefore SingedIntegrals could be represented as
    ...
    // 1. after define an integral constant, say enumerate val, you can not change
    // it
    ...
    // specialization. 这里 primary template中定义出来的template parameter
    // list直接会被omitted
    ...
    // when we use it
    typedef Append<SingedIntegrals, TYPELIST_3(float, double, long double)>::Result
      SignedTypes;
     ...
     *  else
     *    Result is a typelist having TList::Head as its head and the result of
     * applying Erase to TList::Tail and T as its tail
     */
    template <class TList, class T>
    struct Erase;
    ...
    struct Erase<TypeList<Head, Tail>, T> {
      // once we want to remove something, we build a new TypeList and named it as
      // result
