...
namespace Loki
{
  ...
  /**
   * @brief
   * Example: const bool convExists = sizeof(Test(T())) == sizeof(Small), here T() is the type we wants to check, T() is the c'tor
   * - Test(...), is an undefined behavior,  but this doesn't matter. Nothing actually calls the function. It's not even implemented.
   */
  namespace Private
  {
    ...
    struct ConversionHelper
    {
      ...
      // the thought is basically utilizing the compiler to distinguish plateform difference
      // utilize the type of return value to check the result
      // Here is either type Big or type Small
      // to determine if T and U are the same
      // if T and U are exactly the same, they will call the same overloaded function inbetween a. or b.
      // if T and U are diff, they will call the diff
      // These two static functions both stay unimplemented
   ...
   *    return (at compile time) true if there is an implicit conversion from T to U
   *    e.g. Derived to Base
...
# endif
   ...
   * returns true if B is a pub base of D, if B and D area aliases of the same type
   * Core idea: when we pass a pointer,
   * if formal parameter == base pointer
